...
class BuildStagesVi(
    private val buildStagesContext: Context,
    private val buildStagesCallback: BuildStagesCallBack,
    private val buildStagesWindow: Window
) : WebView(buildStagesContext) {
    private var buildStagesFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun buildStagesSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.buildStagesFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(buildStagesContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        buildStagesContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(buildStagesContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    BuildStagesApplication.buildStagesInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(BuildStagesApplication.BUILD_STAGES_MAIN_TAG, "onPageFinished : ${BuildStagesApplication.buildStagesInputMode}")
                    buildStagesWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    buildStagesEnableStableInputScroll()
                } else {
                    BuildStagesApplication.buildStagesInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(BuildStagesApplication.BUILD_STAGES_MAIN_TAG, "onPageFinished : ${BuildStagesApplication.buildStagesInputMode}")
                    buildStagesWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                buildStagesFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                buildStagesHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun buildStagesFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun buildStagesEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun buildStagesHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = BuildStagesVi(buildStagesContext, buildStagesCallback, buildStagesWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            buildStagesCallback.buildStagesHandleCreateWebWindowRequest(windowWebView)
        }
    }
